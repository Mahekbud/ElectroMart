...
def update_user_by_token(request):
    ...
    try:
        user = User.objects.get(pk=user_id_from_token, is_active=True, is_deleted=False) 
...
def delete_user_by_token(request):
    ...
    try:
        user_to_delete = User.objects.get(pk=user_id_from_token, is_active=True, is_deleted=False)
    except User.DoesNotExist:
        return Response({'error': 'User not found'}, status=status.HTTP_404_NOT_FOUND)
